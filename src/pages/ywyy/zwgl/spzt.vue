<template>
  <div>
    ...
    <a-table :columns="columnsData.filter(column => column.visible)" class="iphmdTable" :row-selection="rowSelection"
...
export default {
      ...
      columnsData: [
        {
          title: "id",
          dataIndex: "id",
          className: 'hidden-column', // 默认隐藏此列
        },
        {
          title: "中文名称",
          dataIndex: "name",
  ...
  methods: {
    ...
    async getwjglListData(param) {
      let res = await this.$api.YHGL.get_shen_list({
