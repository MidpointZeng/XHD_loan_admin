    ...
    <a-modal title="新增" :visible="addvisible" :confirm-loading="addconfirmLoading" @ok="addHandleSubmit"
      ...
      <a-form :form="addform" :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }" @submit="addHandleSubmit">
        <a-form-item label="客服名称">
          <a-input placeholder="客服名称" v-decorator="[
              'kefu_name',
        ...
        <a-form-item label="客服地址">
          <a-input placeholder="客服地址" v-decorator="[
              'kefu_url',
               
            ]" />
        </a-form-item>

        <a-form-item label="客服关联域名">
          <a-select placeholder="请选择客服关联域名" v-decorator="[
              'domainName',
            ]"  
        @change="handleKefuUrlChange">
            <a-select-option v-for="item in domainList" :key="item.id" :value="item.domainName">
              {{ item.domainName }}
            </a-select-option>
          </a-select>
        </a-form-item>
...
export default {
  ...
  methods: {
    handleKefuUrlChange(value) {
       debugger;
      console.log('Selected Kefu URL ID:', value);
      this.domainName=value;
    },
 
    async getDomianList() {
      debugger;
      let res = await this.$api.YHGL.Get_domain__Opt({
        method:"select",
        pageNo: 1,
        pageSize: 100,
      });
